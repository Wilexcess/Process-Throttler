...
std::atomic<uint64_t> g_throttleSessionId = 0;

// --- GUI Handles ---
HWND g_hwnd, g_hTabControl, g_hStatusIndicator;
...
HWND g_hTimedModeStatusLabel;
HWND g_hEnableTimedModeCheck, g_hAlwaysOnTopCheck, g_hEnableToggleKeyCheck;
HWND g_hAutoOffCheck, g_hAutoOffMsEdit;
HWND g_hInstructionsText, g_hStatusLabel;
std::vector<HWND> g_settingsControls, g_instructionsControls, g_configControls, g_dynamicControls;

HBRUSH g_hRedBrush, g_hGreenBrush;

// --- Application Settings ---
std::atomic<UINT> g_toggleKey = 'Z';
std::atomic<UINT> g_timedModeToggleKey = 'K';
int g_timedModeMs = 300, g_autoOffMs = 10000;
...
void auto_off_worker(int milliseconds, uint64_t sessionId);
...
bool discover_ports_for_pid(DWORD pid) {
    std::set<UINT16> currentPorts;
    PMIB_TCPTABLE_OWNER_PID tcpTable = NULL; PMIB_UDPTABLE_OWNER_PID udpTable = NULL;
    ...
    if (GetExtendedTcpTable(NULL, &size, TRUE, AF_INET, TCP_TABLE_OWNER_PID_ALL, 0) == ERROR_INSUFFICIENT_BUFFER) {
        ...
        if (tcpTable) { if (GetExtendedTcpTable(tcpTable, &size, TRUE, AF_INET, TCP_TABLE_OWNER_PID_ALL, 0) == NO_ERROR) { for (DWORD i = 0; i < tcpTable->dwNumEntries; i++) { if (tcpTable->table[i].dwOwningPid == pid) { currentPorts.insert(ntohs((u_short)tcpTable->table[i].dwLocalPort)); tcpPortsFound++; } } } free(tcpTable); }
    } size = 0;
    if (GetExtendedUdpTable(NULL, &size, TRUE, AF_INET, UDP_TABLE_OWNER_PID, 0) == ERROR_INSUFFICIENT_BUFFER) {
        ...
        if (udpTable) { if (GetExtendedUdpTable(udpTable, &size, TRUE, AF_INET, UDP_TABLE_OWNER_PID, 0) == NO_ERROR) { for (DWORD i = 0; i < udpTable->dwNumEntries; i++) { if (udpTable->table[i].dwOwningPid == pid) { currentPorts.insert(ntohs((u_short)udpTable->table[i].dwLocalPort)); udpPortsFound++; } } } free(udpTable); }
    }
    if (currentPorts.empty()) { return false; }
    if (currentPorts != g_robloxPorts) {
        g_robloxPorts = currentPorts;
        update_status(L"Discovered " + std::to_wstring(tcpPortsFound) + L" TCP and " + std::to_wstring(udpPortsFound) + L" UDP ports. Ready.");
    }
    return true;
}
...
void delayed_toggle_on_thread() {
    ...
    if (g_isDelayActive.load() && !g_exitSignal.load()) {
        g_isThrottlingActive = true;
        update_status(L"Packet drop ENABLED");
        if (IsDlgButtonChecked(g_hwnd, 404)) {
            wchar_t buffer[20]; GetWindowText(g_hAutoOffMsEdit, buffer, 20); g_autoOffMs = _wtoi(buffer);
            uint64_t newSessionId = ++g_throttleSessionId;
            std::thread(auto_off_worker, g_autoOffMs, newSessionId).detach();
        }
        else { ++g_throttleSessionId; }
    }
    g_isDelayActive = false;
}
void auto_off_worker(int milliseconds, uint64_t sessionId) {
    std::this_thread::sleep_for(std::chrono::milliseconds(milliseconds));
    if (g_exitSignal.load() || g_throttleSessionId.load() != sessionId) return;
    g_isThrottlingActive = false;
    update_status(L"Packet drop auto-disabled.");
}
void input_handler_thread() {
    bool toggle_key_down = false, timed_key_down = false;
    while (!g_exitSignal) {
        if (IsDlgButtonChecked(g_hwnd, 403)) {
            if (GetAsyncKeyState(g_toggleKey.load()) & 0x8000) {
                if (!toggle_key_down) {
                    bool newState = !g_isThrottlingActive.load();
                    g_isThrottlingActive = newState;
                    update_status(newState ? L"Packet drop ENABLED" : L"Packet drop DISABLED");
                    if (newState && IsDlgButtonChecked(g_hwnd, 404)) {
                        wchar_t buffer[20]; GetWindowText(g_hAutoOffMsEdit, buffer, 20); g_autoOffMs = _wtoi(buffer);
                        uint64_t newSessionId = ++g_throttleSessionId;
                        std::thread(auto_off_worker, g_autoOffMs, newSessionId).detach();
                    }
...
LRESULT CALLBACK LowLevelMouseProc(int nCode, WPARAM wParam, LPARAM lParam) {
    if (nCode == HC_ACTION && wParam == WM_LBUTTONDOWN) {
        if (g_isTimedModeEnabled.load()) {
            if (g_isThrottlingActive.load()) {
                g_isThrottlingActive = false; ++g_throttleSessionId;
                update_status(L"Packet drop DISABLED");
            }
...
// *** FIX: Provides continuous feedback that scanning is happening ***
void port_discovery_loop() {
    while (!g_exitSignal) {
        if (g_targetPid != 0) {
            update_status(L"Re-scanning for process ports...");
            if (!discover_ports_for_pid(g_targetPid)) {
                update_status(L"Process running, but no active ports found. Retrying...");
            }
        }
...
int WINAPI wWinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE, _In_ LPWSTR, _In_ int nShowCmd) {
    ...
    g_hRedBrush = CreateSolidBrush(RGB(255, 0, 0)); g_hGreenBrush = CreateSolidBrush(RGB(0, 255, 0));
    ...
    g_hwnd = CreateWindowEx(0, CLASS_NAME, windowTitle.c_str(), WS_OVERLAPPED | WS_CAPTION | WS_SYSMENU | WS_MINIMIZEBOX, CW_USEDEFAULT, CW_USEDEFAULT, 500, 620, NULL, NULL, hInstance, NULL);
    if (g_hwnd == NULL) return 0;
    create_gui_elements(g_hwnd); LoadSettings();
    ShowWindow(g_hwnd, nShowCmd);
    MSG msg = { }; while (GetMessage(&msg, NULL, 0, 0)) { TranslateMessage(&msg); DispatchMessage(&msg); }
    DeleteObject(g_hRedBrush); DeleteObject(g_hGreenBrush);
    return 0;
}
LRESULT CALLBACK KeybindEditProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    switch (uMsg) {
...
LRESULT CALLBACK WindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_APP_STOP_COMPLETE: update_status(L"Stopped. Ready to start."); set_config_controls_state(true, false); return 0;
    case WM_CTLCOLORSTATIC: if ((HWND)lParam == g_hStatusIndicator) { return (LRESULT)(g_isThrottlingActive.load() ? g_hGreenBrush : g_hRedBrush); } break;
    case WM_NOTIFY: if (((LPNMHDR)lParam)->code == TCN_SELCHANGE) { SwitchTab(TabCtrl_GetCurSel(g_hTabControl)); } break;
    case WM_COMMAND: {
        if (HIWORD(wParam) == BN_CLICKED) {
            ...
            if (hButtonClicked == g_hStartButton) { start_throttling(); }
                ...
                g_isThrottlingActive = false; InvalidateRect(g_hStatusIndicator, NULL, TRUE);
                update_status(L"Stopping..."); set_config_controls_state(false, true);
            ...
            else if (hButtonClicked == g_hInboundCheck || hButtonClicked == g_hOutboundCheck || hButtonClicked == g_hBothCheck) {
                g_throttleInbound = IsDlgButtonChecked(hwnd, 201); g_throttleOutbound = IsDlgButtonChecked(hwnd, 202);
                if (hButtonClicked == g_hBothCheck) { bool chk = IsDlgButtonChecked(hwnd, 203); CheckDlgButton(hwnd, 201, chk); CheckDlgButton(hwnd, 202, chk); g_throttleInbound = chk; g_throttleOutbound = chk; }
                else { CheckDlgButton(hwnd, 203, g_throttleInbound && g_throttleOutbound); }
            }
        }
        return 0;
    }
...
void create_gui_elements(HWND hwnd) {
    g_hTabControl = CreateWindow(WC_TABCONTROL, L"", WS_CHILD | WS_VISIBLE, 5, 5, 470, 500, hwnd, NULL, NULL, NULL);
    TCITEM tie; tie.mask = TCIF_TEXT;
    tie.pszText = (LPWSTR)L"Settings"; TabCtrl_InsertItem(g_hTabControl, 0, &tie);
    tie.pszText = (LPWSTR)L"Instructions"; TabCtrl_InsertItem(g_hTabControl, 1, &tie);

    int y = 40; const int ITEM_HEIGHT = 20; const int LABEL_WIDTH = 145; const int EDIT_WIDTH = 100; const int ROW_GAP = 28; const int GROUP_GAP = 30;
    ...
    g_hProcessNameEdit = CreateWindow(L"EDIT", L"", WS_CHILD | WS_BORDER, 160, y, 295, ITEM_HEIGHT, hwnd, NULL, NULL, NULL); y += ROW_GAP;
    ...
    g_settingsControls.push_back(CreateWindow(L"STATIC", L"<- If you're unsure, use Both.", WS_CHILD, 200, y, 255, ITEM_HEIGHT, hwnd, NULL, NULL, NULL)); y += GROUP_GAP;

    g_hEnableToggleKeyCheck = CreateWindow(L"BUTTON", L"Enable Instant Toggle", WS_CHILD | BS_AUTOCHECKBOX, 10, y, 160, ITEM_HEIGHT, hwnd, (HMENU)403, NULL, NULL); y += ROW_GAP;
    g_settingsControls.push_back(CreateWindow(L"STATIC", L"Toggle Keybind:", WS_CHILD, 25, y, 130, ITEM_HEIGHT, hwnd, NULL, NULL, NULL));
    g_hToggleKeyEdit = CreateWindow(L"EDIT", L"", WS_CHILD | WS_BORDER | ES_CENTER | ES_READONLY, 160, y, EDIT_WIDTH, ITEM_HEIGHT, hwnd, (HMENU)101, NULL, NULL); y += GROUP_GAP;

    g_hEnableTimedModeCheck = CreateWindow(L"BUTTON", L"Enable Timed Mode", WS_CHILD | BS_AUTOCHECKBOX, 10, y, 145, ITEM_HEIGHT, hwnd, (HMENU)401, NULL, NULL); y += ROW_GAP;
    g_settingsControls.push_back(CreateWindow(L"STATIC", L"Timed Mode Hotkey:", WS_CHILD, 25, y, 130, ITEM_HEIGHT, hwnd, NULL, NULL, NULL));
    g_hTimedKeyEdit = CreateWindow(L"EDIT", L"", WS_CHILD | WS_BORDER | ES_CENTER | ES_READONLY, 160, y, EDIT_WIDTH, ITEM_HEIGHT, hwnd, (HMENU)102, NULL, NULL);
    g_hTimedModeStatusLabel = CreateWindow(L"STATIC", L"Timed Mode: OFF", WS_CHILD, 270, y, 185, ITEM_HEIGHT, hwnd, NULL, NULL, NULL); y += ROW_GAP;
    g_settingsControls.push_back(CreateWindow(L"STATIC", L"Enter ms:", WS_CHILD, 25, y, 130, ITEM_HEIGHT, hwnd, NULL, NULL, NULL));
    g_hTimedModeMsEdit = CreateWindow(L"EDIT", L"", WS_CHILD | WS_BORDER, 160, y, 50, ITEM_HEIGHT, hwnd, (HMENU)103, NULL, NULL); y += GROUP_GAP;

    g_hAutoOffCheck = CreateWindow(L"BUTTON", L"Automatic Shutoff", WS_CHILD | BS_AUTOCHECKBOX, 10, y, 145, ITEM_HEIGHT, hwnd, (HMENU)404, NULL, NULL); y += ROW_GAP;
    g_settingsControls.push_back(CreateWindow(L"STATIC", L"Turn off after (ms):", WS_CHILD, 25, y, 130, ITEM_HEIGHT, hwnd, NULL, NULL, NULL));
    g_hAutoOffMsEdit = CreateWindow(L"EDIT", L"", WS_CHILD | WS_BORDER, 160, y, 80, ITEM_HEIGHT, hwnd, (HMENU)405, NULL, NULL);
    g_settingsControls.push_back(CreateWindow(L"STATIC", L"(1000ms = 1s)", WS_CHILD, 250, y, 100, ITEM_HEIGHT, hwnd, NULL, NULL, NULL)); y += GROUP_GAP;

    g_hInboundCheck = CreateWindow(L"BUTTON", L"Inbound (Used for COM Offset)", WS_CHILD | BS_AUTOCHECKBOX, 10, y, 210, ITEM_HEIGHT, hwnd, (HMENU)201, NULL, NULL);
    g_hOutboundCheck = CreateWindow(L"BUTTON", L"Outbound", WS_CHILD | BS_AUTOCHECKBOX, 225, y, 80, ITEM_HEIGHT, hwnd, (HMENU)202, NULL, NULL);
    g_hBothCheck = CreateWindow(L"BUTTON", L"Both", WS_CHILD | BS_AUTOCHECKBOX, 310, y, 60, ITEM_HEIGHT, hwnd, (HMENU)203, NULL, NULL);
    g_hAlwaysOnTopCheck = CreateWindow(L"BUTTON", L"Always on Top", WS_CHILD | BS_AUTOCHECKBOX, 361, y, 120, ITEM_HEIGHT, hwnd, (HMENU)402, NULL, NULL);

    const wchar_t* instructions = L"--- General Use ---\r\n1. Enter the process name (e.g., RobloxPlayerBeta.exe).\r\n2. Choose your settings and hotkeys.\r\n3. Press Start.\r\n\r\n--- Instant Mode ---\r\n1. Check \"Enable Instant Toggle\".\r\n2. Press your \"Toggle Keybind\" to instantly turn the lag on or off.\r\n   (This is useful for general purpose lagging).\r\n\r\n--- Timed Mode (Gearless Offset) ---\r\nThis mode is designed for precise actions, like using it for gearless offset of COM.\r\n\r\n1. Configure your desired settings & Check \"Enable Timed Mode\".\r\n2. Press your \"Timed Mode Hotkey\" to \"ARM\" the system. The status will change to \"ON (Click to use)\".\r\n3. In-game, open the menu and hover your mouse over the \"Reset Character\" button.\r\n4. Left-click to reset. The lag will activate automatically after the millisecond delay you set.\r\n5. To disable the lag, simply click your mouse again.";
    g_hInstructionsText = CreateWindow(L"EDIT", instructions, WS_CHILD | ES_MULTILINE | ES_READONLY | WS_VSCROLL, 10, 40, 455, 450, hwnd, NULL, NULL, NULL);

    g_hStatusLabel = CreateWindow(L"STATIC", L"Status: Idle. Press Start.", WS_VISIBLE | WS_CHILD, 10, 510, 250, 20, hwnd, NULL, NULL, NULL);
    g_hStatusIndicator = CreateWindow(L"STATIC", L"", WS_VISIBLE | WS_CHILD, 265, 510, 20, 20, hwnd, NULL, NULL, NULL);
    CreateWindow(L"STATIC", L"Made By Wilexcess :)", WS_VISIBLE | WS_CHILD | SS_RIGHT, 300, 510, 170, 20, hwnd, NULL, NULL, NULL);
    g_hStartButton = CreateWindow(L"BUTTON", L"Start", WS_VISIBLE | WS_CHILD | BS_DEFPUSHBUTTON, 130, 540, 100, 30, hwnd, (HMENU)1, NULL, NULL);
    g_hStopButton = CreateWindow(L"BUTTON", L"Stop", WS_VISIBLE | WS_CHILD, 250, 540, 100, 30, hwnd, (HMENU)2, NULL, NULL);

    g_settingsControls.insert(g_settingsControls.end(), { g_hProcessNameEdit, g_hProtoTCP, g_hProtoUDP, g_hProtoBoth, g_hEnableToggleKeyCheck, g_hToggleKeyEdit, g_hEnableTimedModeCheck, g_hTimedKeyEdit, g_hTimedModeStatusLabel, g_hTimedModeMsEdit, g_hInboundCheck, g_hOutboundCheck, g_hBothCheck, g_hAlwaysOnTopCheck, g_hAutoOffCheck, g_hAutoOffMsEdit });
    g_instructionsControls.push_back(g_hInstructionsText);
    g_configControls = { g_hProcessNameEdit, g_hProtoTCP, g_hProtoUDP, g_hProtoBoth, g_hEnableToggleKeyCheck, g_hToggleKeyEdit, g_hEnableTimedModeCheck, g_hTimedKeyEdit, g_hTimedModeMsEdit, g_hAlwaysOnTopCheck, g_hAutoOffCheck, g_hAutoOffMsEdit };
    g_dynamicControls = { g_hInboundCheck, g_hOutboundCheck, g_hBothCheck }; // Controls that can be changed while running
...
void SaveSettings() {
    ...
    GetWindowText(g_hAutoOffMsEdit, buffer, 256); WritePrivateProfileString(L"Settings", L"AutoOffMs", buffer, g_iniPath);
    ...
    wsprintf(buffer, L"%d", IsDlgButtonChecked(g_hwnd, 404) ? 1 : 0); WritePrivateProfileString(L"Settings", L"EnableAutoOff", buffer, g_iniPath);
}
void LoadSettings() {
    ...
    vkCode = GetPrivateProfileInt(L"Settings", L"TimedKey", 'K', g_iniPath); g_timedModeToggleKey = vkCode;
    ...
    int timedMs = GetPrivateProfileInt(L"Settings", L"TimedMs", 300, g_iniPath); wsprintf(buffer, L"%d", timedMs); SetWindowText(g_hTimedModeMsEdit, buffer);
    int autoOffMs = GetPrivateProfileInt(L"Settings", L"AutoOffMs", 10000, g_iniPath); wsprintf(buffer, L"%d", autoOffMs); SetWindowText(g_hAutoOffMsEdit, buffer);
    ...
    bool enableTimed = GetPrivateProfileInt(L"Settings", L"EnableTimedMode", 0, g_iniPath) == 1; CheckDlgButton(g_hwnd, 401, enableTimed ? BST_CHECKED : BST_UNCHECKED); set_timed_mode_controls_state(enableTimed);
    bool alwaysOnTop = GetPrivateProfileInt(L"Settings", L"AlwaysOnTop", 0, g_iniPath) == 1; CheckDlgButton(g_hwnd, 402, alwaysOnTop ? BST_CHECKED : BST_UNCHECKED); if (alwaysOnTop) { SetWindowPos(g_hwnd, HWND_TOPMOST, 0, 0, 0, 0, SWP_NOMOVE | SWP_NOSIZE); }
    bool enableToggle = GetPrivateProfileInt(L"Settings", L"EnableToggleKey", 1, g_iniPath) == 1; CheckDlgButton(g_hwnd, 403, enableToggle ? BST_CHECKED : BST_UNCHECKED);
    bool enableAutoOff = GetPrivateProfileInt(L"Settings", L"EnableAutoOff", 0, g_iniPath) == 1; CheckDlgButton(g_hwnd, 404, enableAutoOff ? BST_CHECKED : BST_UNCHECKED);
}
void update_status(const std::wstring& status, bool isSubStatus) {
    ...
    else { SetWindowText(g_hStatusLabel, (L"Status: " + status).c_str()); InvalidateRect(g_hStatusIndicator, NULL, TRUE); }
}
void set_config_controls_state(bool enabled, bool stopping) {
    ...
    else { set_timed_mode_controls_state(false); for (HWND hCtrl : g_dynamicControls) EnableWindow(hCtrl, TRUE); }
    EnableWindow(g_hStartButton, enabled); EnableWindow(g_hStopButton, !enabled);
    if (stopping) { EnableWindow(g_hStopButton, FALSE); }
}
void set_timed_mode_controls_state(bool enabled) {
    EnableWindow(g_hTimedKeyEdit, enabled); EnableWindow(g_hTimedModeMsEdit, enabled);
...
void stop_throttling_worker() {
    ...
    if (g_hwnd) { PostMessage(g_hwnd, WM_APP_STOP_COMPLETE, 0, 0); }
}
void SwitchTab(int tabIndex) {
